...
class NowPlayingWidgetActionReceiver : BroadcastReceiver() {
  override fun onReceive(context: Context, intent: Intent?) {
    ...
    when (action) {
      "sc.pirate.app.WIDGET_PLAY_PAUSE" -> player.togglePlayPause()
      "sc.pirate.app.WIDGET_SKIP_NEXT" -> player.skipNext()
    }
  }

  companion object {
    @Volatile
    var playerRef: sc.pirate.app.player.PlayerController? = null
  }
}
